...
int agregarPelicula(EMovie* movie, EMovie* auxVector,int* cantidad)
{
     char resp='s',auxCad[800];
     int auxInt,i,cont=0;
     do
      {
        system("cls");
       ...
       (movie + cont)->activeStatus = 1;
        printf("Desea agregar mas Peliculas [s/n]? ");
        fflush(stdin);
        resp = tolower(getchar());
        while(resp!='s' && resp!='n')
         {
           printf("Respuesta incorrecta, desea agregar mas Peliculas [s/n]? ");
           fflush(stdin);
           resp = tolower(getchar());
         }
         cont++;
         ...
         {
           //cont++;
           auxVector = (EMovie*)realloc(movie,sizeof(EMovie)*(cont+1));
           //movie = auxVector;
         }
         //movie = auxVector;
      }while(resp == 's');
      ...
      for(i=0;i<cont;i++)
       {
          if((movie + i)->activeStatus == -1)
            continue;
          printf("Titulo: %s\tEstado: %d\n",(movie + i)->titulo,(movie + i)->activeStatus);
       }
      *cantidad = cont;
   return 1;
}
...
void generarPagina(EMovie lista[], char nombre[],int cant_elem)
{
  ...
  for(i=0;i<cant_elem;i++)
  {
    if(lista[i].activeStatus != 1)
       continue;
      fprintf(fd,"<div class='row'>     <article class='col-md-4 article-intro'><a href='#'><img class='img-responsive img-rounded' src='%s' alt='Error durante la carga de la imagen'>",(lista + i)->linkImagen);
      fprintf(fd,"</a>");
      fprintf(fd,"<h3>");
